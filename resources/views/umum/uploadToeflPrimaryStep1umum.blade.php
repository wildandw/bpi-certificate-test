...
<head>
  ...
  <title>Upload Data Skor TOEFL Primary Step 1 - Umum</title>
  <!-- Bootstrap 5 CSS CDN -->
...
<body>
  <div class="container">
    <div class="upload-container">
      <h1 class="text-center">TOEFL Primary Step 1 Raw Score Conversion - <b>Umum</b></h1>
    ...
    @endif
      ...
      <form action="{{ route('scores.importToeflPrimaryStep1umum') }}" method="POST" enctype="multipart/form-data">
        @csrf
        ...
        <!-- no sertif -->
        <div class="mb-3">
          <label for="no_sertif" class="form-label">Certificate Number</label>
          <input type="text" name="no_sertif" id="no_sertif" class="form-control" required>
        </div>
        <!-- valid until -->
        <div class="mb-3">
          <label for="valid_date" class="form-label">Valid Until</label>
      ...
      @if($hasConversion)
        <form action="{{ route('scoreconversiontoeflprimarystep1.reset') }}" method="POST" class="mt-3" onsubmit="return confirm('Yakin ingin me-reset score conversion toefl primary step1 ? Data akan dihapus seluruhnya.')">
          @csrf
          @method('DELETE')
